...
public class Bill_ManagementAdmin extends JFrame {
	...
	public static void setUser(User user) {
		Bill_ManagementAdmin.user = user;
	}
	/**
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Bill_ManagementAdmin frame = new Bill_ManagementAdmin();
	...
	public Bill_ManagementAdmin() {
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					if(rdSelected){
						table.setModel(um.searchBill("id", key));
						System.out.println(key);
					} 
				
					else{
					//int key2 = Integer.parseInt(textField_4.getText().toString());
					table.setModel(um.searchBill("name_client", key));
					System.out.println(key);
					}
				}catch (Exception e) {
					JOptionPane.showMessageDialog(null, e.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);
