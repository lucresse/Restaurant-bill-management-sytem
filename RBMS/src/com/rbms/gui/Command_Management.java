...
public class Command_Management extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					Command_Management frame = new Command_Management();
	...
	public static void setUser(User user) {
		Command_Management.user = user;
	}

	/**
	 * Create the frame.
	 */
	public Command_Management() {
		setResizable(false);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					tm.addCommand(cm);
					table.setModel(um.showCommand(user.getName()));
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					tm.updateCommand(cm);
					table.setModel(um.showCommand(user.getName()));
					table_1.setModel(um.showCommand(user.getName()));
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String key=textField_4.getText().toString();
					try {
						if(rdSelected){
							table.setModel(um.searchCommand("name_client", key,user.getName()));
							System.out.println(key);
						} 
					
						else{
						//int key2 = Integer.parseInt(textField_4.getText().toString());
						table.setModel(um.searchCommand("id", key,user.getName()));
						System.out.println(key);
						}
					}catch (Exception e) {
						JOptionPane.showMessageDialog(null, e.getMessage(),"Error",JOptionPane.ERROR_MESSAGE);
